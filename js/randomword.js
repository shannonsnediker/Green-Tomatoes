async function grabWord() {
  ...
  let fetchOptions = {
    // Setting variable equal to the get method that pulls specified data from API
    method: "GET",
    headers: {
      // Headers for authentication and host
...
function selectPoemWords(array) {
  //created a function with one argument that randomly selects a word from an arary and removes that selected word using the splice funciton. This funciton contains one parameter so that each object property can be passed through.
...
let loader = document.getElementById("loader"); // setting loader animation equal to variable loader

let beginButton = document.getElementById("beginButton"); // setting start button that displays the words from array displayWord to variable beginButton

beginButton.style.display = "none"; // removing visibility of start button while API call executes 30 times

async function runForLoops() {
  for (let i = 0; i < 30; i++) {
    //created a for loop to iterate through the desired array and select 30 words from the object.primary property

    if (primary.length > 0) {
      displayWord.push(selectPoemWords(primary)); // passes the words.primary as an argument of the selectPoemWords function to randomly select the 30 words and then push those words to the displayWord array
      loader.style.display = "none"; // once all the words have been pushed the loader animation is removed
      beginButton.style.display = "block"; //once all the words have been pushed the begin button displays
    }
  }

  for (let i = 0; i < 20; i++) {
    //created a for loop to iterate through the desired array and select 20 words from the object.secondary property
  ...
  for (let i = 0; i < 10; i++) {
    //created a for loop to iterate through the desired array and select 10 words from the object.tertiary property
...
async function callAwaitFunctions() {
  for (let i = 0; i < 30; i++) {
    //created a for loop to loop thorugh api 30 times and pushes to words.primary array
    await grabWord(); // waits for each api call to finish before starting the next one
  }
  await runForLoops(); // after all 30 words are fetched the for loops to push the words to displayWords array starts
}

callAwaitFunctions();

function outputDisplayWord() {
  // created a function to wrap each random word that was pushed to the displayWord array in a div, outputted to the dom, and then add an event listener so that each word is clickable.
  ...
  displayWord.forEach((arrayItem) => {
    ...
    node.addEventListener("click", function () {
      // adds a click event listener to each displayWord array element that then activates the addToPoem function passing that same array element as an argument
      addToPoem(arrayItem);
    });
  });
}
...
function deleteLastWord() {
  // function to delete the last word in the array
  if (poem.length > 0) {
...
document
  ...
  .addEventListener("click", deleteLastWord); // added an event listener to the delete button so that it calls the deleteLastWord() function on click
